...
{
	template <class FRM>
	constexpr WAV<FRM> processFIRCombFilter(const WAV<FRM>& wav, FQ fq, value_t alpha, bool fixedLength = true)
	{
	...
	template <class FRM>
	constexpr WAV<FRM> processFIRCombFilter(const WAV<FRM>& wav, const WAV<FQ>& fq, value_t alpha, bool fixedLength = true)
	{
	...
	template <class FRM>
	constexpr WAV<FRM> processFIRCombFilter(const WAV<FRM>& wav, FQ fq, const WAV<value_t>& alpha, bool fixedLength = true)
	{
		const XTM xtm = 1.0 / fq;
		const NTM wavLength = wav.getLength() + static_cast<NTM>(deeproot::ceil(xtm));
		WAV<FRM> ret(wavLength);
	...
	template <class FRM>
	constexpr WAV<FRM> processFIRCombFilter(const WAV<FRM>& wav, const WAV<FQ>& fq, const WAV<value_t>& alpha, bool fixedLength = true)
	{
		const NTM operandLength = deeproot::max(wav.getLength(), fq.getLength());
		const NTM wavLength = wav.getLength() + static_cast<NTM>(deeproot::ceil(1.9 / fq(operandLength - 1)));
		WAV<FRM> ret(wavLength);
